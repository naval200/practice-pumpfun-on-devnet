...
import { PUMP_PROGRAM_ID, COMPUTE_BUDGET_PROGRAM_ID, GLOBAL_VOLUME_ACCUMULATOR, TOKEN_PROGRAM_ID, GLOBAL_SEED, BONDING_CURVE_SEED, CREATOR_VAULT_SEED, GLOBAL_VOLUME_ACCUMULATOR_SEED, USER_VOLUME_ACCUMULATOR_SEED, EVENT_AUTHORITY_SEED, COMPUTE_BUDGET_INSTRUCTIONS, } from './constants.js';
...
export function deriveCreatorVaultAddress(creator) {
    return PublicKey.findProgramAddressSync([CREATOR_VAULT_SEED, creator.toBuffer()], PUMP_PROGRAM_ID);
}
/**
...
export function deriveUserVolumeAccumulatorAddress(user) {
    return PublicKey.findProgramAddressSync([USER_VOLUME_ACCUMULATOR_SEED, user.toBuffer()], PUMP_PROGRAM_ID);
}
/**
 ...
 */
export function deriveGlobalAddress() {
 ...
 */
export async function validateBondingCurve(connection, mint) {
    try {
 ...
 */
export async function getBondingCurveAccount(connection, mint, commitment = 'confirmed') {
 ...
 */
export function calculateWithSlippageBuy(solAmount, slippageBasisPoints) {
    return (solAmount * (10000n + slippageBasisPoints)) / 10000n;
}
/**
 * Calculate sell amount with slippage
 */
export function calculateWithSlippageSell(solAmount, slippageBasisPoints) {
    return (solAmount * (10000n - slippageBasisPoints)) / 10000n;
}
// ============================================================================
 ...
 */
export function getGlobalAccountPDA() {
 ...
 */
export function getGlobalIncentiveTokenAccountPDA(mint) {
    const [globalIncentiveTokenAccount] = PublicKey.findProgramAddressSync([getGlobalVolumeAccumulatorPDA().toBuffer(), TOKEN_PROGRAM_ID.toBuffer(), mint.toBuffer()], PUMP_PROGRAM_ID);
 ...
 */
export function getUserVolumeAccumulator(programId, user) {
    const [pda] = PublicKey.findProgramAddressSync([USER_VOLUME_ACCUMULATOR_SEED, user.toBuffer()], programId);
 ...
 */
export function getCreatorVaultPDAFromWallet(programId, wallet) {
    const [pda] = PublicKey.findProgramAddressSync([CREATOR_VAULT_SEED, wallet.toBuffer()], programId);
 ...
 */
export function getAllRequiredPDAsForBuy(programId, mint, user) {
    // Global PDA
 ...
 */
export function getGlobalPDA(programId) {
 ...
 */
export async function isGlobalAccountInitialized(connection, programId) {
    try {
        const globalPDA = getGlobalPDA(programId);
 ...
 */
export async function initializeGlobalAccount(connection, wallet, programId) {
    try {
