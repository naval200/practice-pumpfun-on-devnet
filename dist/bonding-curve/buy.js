...
import { PUMP_PROGRAM_ID, FEE_RECIPIENT, GLOBAL_VOLUME_ACCUMULATOR, TOKEN_PROGRAM_ID, SYSTEM_PROGRAM_ID, BUY_INSTRUCTION_DISCRIMINATOR, } from './constants';
...
async function createCompleteBuyInstruction(programId, buyer, mint, solAmount, maxSlippageBasisPoints = 1000) {
    ...
    const { globalPDA, bondingCurvePDA, creatorVaultPDA, eventAuthorityPDA, globalVolumeAccumulatorPDA, userVolumeAccumulatorPDA, } = getAllRequiredPDAsForBuy(programId, mint, buyer);
 ...
 */
export async function buyPumpFunToken(connection, wallet, mint, solAmount, slippageBasisPoints = 1000) {
    ...
    while (attempts < maxAttempts) {
        ...
        try {
            const buyInstruction = await createCompleteBuyInstruction(PUMP_PROGRAM_ID, wallet.publicKey, mint, new BN(solAmount * 1e9), // Convert SOL to lamports
            slippageBasisPoints);
