...
async function getCreateInstructions(program, creator, name, symbol, uri, mint) {
    ...
    const [bondingCurvePDA] = PublicKey.findProgramAddressSync([Buffer.from(BONDING_CURVE_SEED), mint.publicKey.toBuffer()], program.programId);
    // FIX: Use getAssociatedTokenAddressSync with allowOwnerOffCurve = true
    const associatedBondingCurve = getAssociatedTokenAddressSync(mint.publicKey, bondingCurvePDA, true // allowOwnerOffCurve = true for program-owned accounts
    );
 ...
 */
export async function createPumpFunToken(connection, wallet, tokenConfig, isMainnet = false) {
    try {
        ...
        if (!(await isGlobalAccountInitialized(connection, programId))) {
            ...
            if (!globalInitResult.success) {
                ...
                log('💡 The PumpFun program requires a global account to be initialized before creating tokens.');
        ...
        try {
            if (imageFile) {
                const metadata = await uploadMetadata(tokenConfig.name, tokenConfig.symbol, tokenConfig.description, imageFile);
        ...
        if (tokenConfig.initialBuyAmount && tokenConfig.initialBuyAmount > 0) {
            ...
            try {
                ...
                const buySignature = await buyPumpFunToken(connection, wallet, mint.publicKey, buyAmountSol, 1000 // slippageBasisPoints
                );
