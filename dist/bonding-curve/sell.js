...
import { deriveBondingCurveAddress, getAllRequiredPDAsForBuy, // Can reuse this for sell
 } from './helper';
import { PUMP_PROGRAM_ID, FEE_RECIPIENT, TOKEN_PROGRAM_ID, SYSTEM_PROGRAM_ID, SELL_INSTRUCTION_DISCRIMINATOR, } from './constants';
...
async function createCompleteSellInstruction(programId, seller, mint, tokenAmount) {
    ...
    const { globalPDA, bondingCurvePDA, creatorVaultPDA, eventAuthorityPDA } = getAllRequiredPDAsForBuy(programId, mint, seller);
    // Get associated token addresses
    const { getAssociatedTokenAddressSync } = await import('@solana/spl-token');
    const associatedBondingCurve = getAssociatedTokenAddressSync(mint, bondingCurvePDA, true // allowOwnerOffCurve for program accounts
    );
 ...
 */
export async function sellPumpFunToken(connection, wallet, mint, tokenAmount) {
    ...
    const userAtaResult = await getOrCreateAssociatedTokenAccount(connection, wallet, wallet.publicKey, mint);
    ...
    const bondingCurveAtaResult = await getOrCreateAssociatedTokenAccount(connection, wallet, bondingCurve, mint, true // allowOwnerOffCurve
    );
    ...
    while (attempts < maxAttempts) {
        ...
        try {
            const sellInstruction = await createCompleteSellInstruction(PUMP_PROGRAM_ID, wallet.publicKey, mint, new BN(tokenAmount) // Token amount to sell
            );
 ...
 */
export async function sellAllPumpFunTokens(connection, wallet, mint) {
 ...
 */
export async function sellPercentagePumpFunTokens(connection, wallet, mint, percentage // 0-100
) {
