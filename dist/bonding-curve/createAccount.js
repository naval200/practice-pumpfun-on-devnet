...
import { getAssociatedTokenAddress, getAssociatedTokenAddressSync, createAssociatedTokenAccountInstruction, getAccount, } from '@solana/spl-token';
 ...
 */
export async function createAssociatedTokenAccount(connection, payer, owner, mint, allowOwnerOffCurve = false) {
    try {
        ...
        createAtaTx.add(createAssociatedTokenAccountInstruction(payer.publicKey, // payer
 ...
 */
export async function getOrCreateAssociatedTokenAccount(connection, payer, owner, mint, allowOwnerOffCurve = false) {
    try {
        // Use getAssociatedTokenAddressSync for program-owned accounts
        const userTokenAccount = allowOwnerOffCurve
            ? getAssociatedTokenAddressSync(mint, owner, allowOwnerOffCurve)
 ...
 */
export async function checkAssociatedTokenAccountExists(connection, owner, mint) {
    try {
 ...
 */
export async function getAssociatedTokenBalance(connection, owner, mint) {
    try {
        const userTokenAccount = await getAssociatedTokenAddress(mint, owner);
