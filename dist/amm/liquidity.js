 ...
 */
export async function addLiquidity(connection, wallet, poolKey, baseAmount, slippage = 1) {
    try {
        ...
        const liquiditySolanaState = await retryWithBackoff(async () => {
        ...
        const { quote, lpToken } = await retryWithBackoff(async () => {
        ...
        const depositInstructions = await retryWithBackoff(async () => {
 ...
 */
export async function removeLiquidity(connection, wallet, poolKey, lpTokenAmount, slippage = 1) {
    try {
        ...
        const { base, quote } = await retryWithBackoff(async () => {
            // Convert to BN for SDK compatibility
        ...
        const withdrawInstructions = await retryWithBackoff(async () => {
            // Convert to BN for SDK compatibility
            const lpTokenAmountBN = new BN(lpTokenAmount);
