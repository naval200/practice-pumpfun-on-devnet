 ...
 */
export async function getPoolCreationData(pumpAmmSdk, index, creator, baseMint, quoteMint, baseIn, quoteIn) {
    ...
    debugLog(`   Quote amount: ${quoteIn} SOL -> ${quoteInLamports} lamports -> ${quoteInBN.toString()}`);
 ...
 */
export async function findPoolsForToken(connection, tokenMint) {
    try {
        debugLog(`🔍 Searching for AMM pools for token: ${tokenMint.toString()}`);
        // Import the SDK constants and utilities
        const { PUMP_AMM_PROGRAM_ID, poolPda, canonicalPumpPoolPda } = await import('@pump-fun/pump-swap-sdk');
        ...
        try {
            ...
            try {
                ...
                debugLog(`   ❌ Canonical pool not accessible: ${error.message}`);
            }
        }
        catch (error) {
            debugLog(`   ⚠️ Error with canonical pool: ${error.message}`);
        }
        ...
        for (const index of commonIndices) {
            try {
                ...
                try {
                    ...
                    debugLog(`   ❌ Pool ${index} not accessible: ${error.message}`);
                }
            }
            catch (error) {
                debugLog(`   ⚠️ Error with pool index ${index}: ${error.message}`);
            }
        }
        ...
        if (foundPools.length === 0) {
            ...
            try {
                ...
                const allAccounts = await connection.getProgramAccounts(new PublicKey(PUMP_AMM_PROGRAM_ID), {
                ...
                debugLog(`   🔍 Found ${potentialPools.length} potential pool(s) in program account search`);
                ...
                debugLog(`   ⚠️ Program account search failed: ${error.message}`);
            }
        }
 ...
 */
export async function checkAMMPoolLiquidity(connection, tokenMint) {
    try {
 ...
 * Get AMM pool info
 */
export async function getAMMPoolInfo(connection, tokenMint) {
    try {
