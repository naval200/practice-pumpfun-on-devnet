 ...
 */
export async function createPool(connection, wallet, baseMint, quoteMint, baseIn, quoteIn, index = 0) {
    try {
        ...
        const { createPoolSolanaState, createPoolInstructions, initialPoolPrice } = await retryWithBackoff(async () => {
            return await getPoolCreationData(pumpAmmSdk, index, wallet.publicKey, baseMint, quoteMint, baseIn, quoteIn);
